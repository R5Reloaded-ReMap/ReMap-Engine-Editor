...
namespace CodeViews
{
    ...
    public class CodeViewsMenu
    {
        ...
        private static bool boolRef = true;
        private static int intRef = 1;

        private static Vector3 vecRef = new(0, 0, 0);
        //
        ...
        internal static FunctionRef[] LargeFieldPreview =
        {
            () => OptionalTextField( ref strRef, "text ref", "text field" ),
            () => OptionalToggle( ref boolRef, "toggle ref", "toggle field" ),
            () => OptionalIntField( ref intRef, "int ref", "int field" ),
            () => OptionalVector3Field( ref vecRef, "vector ref", "vector field" ),
            () => OptionalTextInfo( "info ref", "info field" ),
            () => OptionalButton( "button ref", "button field", EmptyFunctionRef ),

            () => CreateMenu( "MediumFieldPreview", MediumFieldPreview, MenuType.Medium, "Medium Field Preview", "Medium Field Preview", "", false, false )
        };

        internal static FunctionRef[] MediumFieldPreview =
        {
        ...
        internal static FunctionRef[] SmallFieldPreview =
        {
        ...
        {
            var list = new List< FunctionRef >();

            foreach ( var color in CodeViewsWindow.Color_Array )
                list.Add( () => OptionalButton( color.Key, "", () => { CodeViewsWindow.ephemeralMessage.AddToQueueMessage( $"#RMAPDEV_MESSAGE_{color.Key}", $"{color.Key} Notification", 4, true, color.Value ); }, null,
                    MenuType.Medium ) );
        ...
        {
            CreateMenu( CodeViewsWindow.SelectionMenu, EmptyFunctionRefArray, MenuType.Large, "Disable Selection Only", "Enable Selection Only", "If true, generates the code of the selection only", true );
        }
        ...
        internal static MenuInit CreateMenu( string name, FunctionRef functionRef, MenuType menuType = MenuType.Large, string trueText = "", string falseText = "", string tooltip = "", bool refresh = false, bool enableSeparator = true,
            bool condition = true, bool isButton = false )
        {
            return CreateMenu( name, new[] { functionRef }, menuType, trueText, falseText, tooltip, refresh, enableSeparator, condition, isButton );
        }
            ...
            bool condition = true, bool isButton = false )
        {
            ...
            {
                menu = new MenuInit( name, functionRef, menuType );
            }
            ...
            var buttonContent = new GUIContent( menu.IsOpen ? trueText : falseText, tooltip );
            var buttonContentInfo = new GUIContent( menu.IsOpen ? CodeViewsWindow.enableLogo : CodeViewsWindow.disableLogo, tooltip );
        ...
        internal static void OptionalAdvancedOption()
        {
            ...
            foreach ( var objectType in Helper.ObjectsToHide.Keys )
            {
        ...
        internal static void VerifyGenerateObjects()
        {
            ...
            float progress = 0.0f;
            int min = 0;
            int max = Helper.GenerateObjects.Keys.Count;

            foreach ( var objectType in Helper.GenerateObjects.Keys )
            {
        ...
        internal static void OptionalAdditionalCodeOption()
        {
            ...
            if ( CodeViewsWindow.ShowFunctionEnable() )
                foreach ( var content in AdditionalCodeTab.additionalCode.HeadContent.Content )
                    CreateMenu( $"{content.Name}_HeadContent", () => AdditionalCodeBoolChange( content.Name, "HeadContent", ref CodeViewsWindow.additionalCodeHead, AdditionalCodeTab.additionalCode.HeadContent.Content ), MenuType.Small,
                        content.Name, content.Name, "", false, false, true, true );
            ...
            foreach ( var content in AdditionalCodeTab.additionalCode.InBlockContent.Content )
                CreateMenu( $"{content.Name}_InBlockContent", () => AdditionalCodeBoolChange( content.Name, "InBlockContent", ref CodeViewsWindow.additionalCodeInBlock, AdditionalCodeTab.additionalCode.InBlockContent.Content ),
                    MenuType.Small, content.Name, content.Name, "", false, false, true, true );
            ...
            if ( CodeViewsWindow.ShowFunctionEnable() )
                foreach ( var content in AdditionalCodeTab.additionalCode.BelowContent.Content )
                    CreateMenu( $"{content.Name}_BelowContent", () => AdditionalCodeBoolChange( content.Name, "BelowContent", ref CodeViewsWindow.additionalCodeBelow, AdditionalCodeTab.additionalCode.BelowContent.Content ),
                        MenuType.Small, content.Name, content.Name, "", false, false, true, true );
            else OptionalTextInfo( "\"Show Squirrel Function\" is disable", "", null, MenuType.Small );
            ...
            if ( isFirstOpen )
            {
                ...
                {
                    var menu = MenuInit.Find( $"{AdditionalCodeTab.emptyContentStr}_{type}" );
        ...
        private static void AdditionalCodeBoolChange( string name, string type, ref string codeRef, List< AdditionalCodeContent > contents )
        {
            if ( contents.Count == 1 )
            {
                var umenu = MenuInit.Find( $"{name}_{type}" );
            ...
            {
                var menu = MenuInit.Find( $"{content.Name}_{type}" );
    ...
    {
        public static Dictionary< string, MenuInit > MenuDictionary = new();
        ...
        {
            return MenuDictionary.TryGetValue( name, out var menu ) ? menu : null;
        }
