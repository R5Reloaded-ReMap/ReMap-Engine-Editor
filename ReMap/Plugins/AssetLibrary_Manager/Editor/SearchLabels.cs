...
namespace AssetLibraryManager
{
    ...
    {
        public static List< string > selectedLabels = new();

        public static List< string > cacheGuids = new();
        public static List< Texture2D > cacheThumbs = new();

        public static List< Object > selectedPrefabs = new();
        ...
        public static void GetGUIDs()
        {
            ...
            if ( cacheGuids.Count == 0 )
            {
                ...
                if ( PrefabLabels.settings.includedFolders.Count == 0 )
                    cacheGuids.AddRange( AssetDatabase.FindAssets( "t:prefab", new[] { "Assets" } ).ToList() );
                else
                    ...
                    for ( int i = 0; i < PrefabLabels.settings.includedFolders.Count; i++ )
                    {
                        ...
                        if ( path != string.Empty )
                            ...
                            cacheGuids.AddRange( AssetDatabase.FindAssets( "t:prefab", new[] { path } ).ToList() );
                    }
                ...
                for ( int i = 0; i < PrefabLabels.settings.excludedFolders.Count; i++ )
                {
                    ...
                    if ( path != string.Empty )
                        guidsToRemove.AddRange( AssetDatabase.FindAssets( "t:prefab", new[] { path } ).ToList() );

                    //Remove the ones in excluded folders
                    foreach ( string item in guidsToRemove )
        ...
        public static void Search()
        {
            ...
            if ( selectedLabels.Count != 0 )
            {
                ...
                //Search Method "ALL"
                    ...
                    {
                        var assetGUID = new GUID( cacheGuids[i] );
        ...
        public static void LoadAllPrefabs()
        {
            ...
            for ( int i = 0; i < cacheGuids.Count; i++ )
            {
                var guid = new GUID( cacheGuids[i] );
