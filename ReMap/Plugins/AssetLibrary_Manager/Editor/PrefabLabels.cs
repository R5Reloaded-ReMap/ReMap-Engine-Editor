...
namespace AssetLibraryManager
{
    public class PrefabLabels : EditorWindow
    {
        ...
        public static List< string > customLabels = new();
        public static List< string > isPressed = new();

        public static int searchMethod, oldSearchMethod;

        private bool customFold = true;

        private Vector2 scrollPosition;
        private readonly string[] searchMode = { "ANY", "ALL" };
        ...
        private void OnGUI()
        {
            ...
            if ( searchMethod != oldSearchMethod )
            {
                ...
                if ( settings.options.displayAllPrefabs )
        ...
        private void CreateSection( GUIStyle guiStyle )
        {
            for ( int i = 0; i < settings.createNewSection.Count; i++ )
            {
                //Get each sections
                var createNewTab = settings.createNewSection[i];
                    ...
                    if ( createNewTab.fold )
                    {
                        ...
                        if ( createNewTab.labelIndex != createNewTab.oldLabelIndex )
                        {
                            ...
                            if ( searchMethod == 1 )
                            {
                                ...
                                foreach ( string label in isPressed )
        ...
        private void Custom_Labels_btn( GUIStyle guiStyle )
        {
        ...
        public static void SetUp_Labels()
        {
            ...
            foreach ( string id in SearchLabels.cacheGuids )
            {
                var guid = new GUID( id );

                var getLabels = AssetDatabase.GetLabels( guid ).ToList();
            ...
            for ( int a = 0; a < settings.createNewSection[i].sectionLabels.Count; a++ )
                if ( settings.createNewSection[i].hideFromLabels )
