...
namespace Build
{
    public class BuildNonVerticalZipline
    {
        public static async Task< StringBuilder > BuildNonVerticalZipLineObjects( GameObject[] objectData, BuildType buildType )
        {
            ...
            {
                var script = ( DrawNonVerticalZipline )Helper.GetComponentByEnum( obj, ObjectType.NonVerticalZipLine );
                ...
                switch ( buildType )
                {
                    ...
                    case BuildType.LiveMap:
                        ...
                        LiveMap.AddToGameQueue( $"External_ReMapSetBool([false,{Helper.BoolToLower( script.RestPoint )},{Helper.BoolToLower( script.IsMoving )}])" );
