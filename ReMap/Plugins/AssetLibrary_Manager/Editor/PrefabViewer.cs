...
namespace AssetLibraryManager
{
    public class PrefabViewer : EditorWindow
    {
        ...
        private readonly int edgePadding = 5;

        private Texture2D placeHolder;

        private Vector2 scrollView;
        private readonly int thumbSpacing = 2;
        ...
        private void CreateGUI()
        {
            ...
            if ( !HasOpenInstances< PrefabLabels >() )
                if ( SearchLabels.selectedLabels.Count == 0 && settings.options.displayAllPrefabs )
                {
        ...
        public static void SceneGUI( SceneView sceneView )
        {
            ...
            if ( Event.current.control && Event.current.shift )
            {
                ...
                {
                    if ( settings.placement.useScale )
                    {
                        if ( settings.placement.useScaleSnap )
                        {
                    ...
                    if ( settings.placement.useRotation )
                    {
                        if ( settings.placement.useRotationSnap )
                        {
                            if ( Time.frameCount % settings.placement.mouseSensibility == 0 )
                            {
                                float rot = item.transform.localEulerAngles.y;
                                var angle = item.transform.localEulerAngles;
            ...
            scrollView = GUILayout.BeginScrollView( scrollView );
            {
                ...
                for ( int i = 1; i < SearchLabels.selectedPrefabs.Count; i++ )
                {
                    ...
                    if ( Event.current.type == EventType.MouseDown )
                    {
                        ...
                        DragAndDrop.objectReferences = new[] { SearchLabels.selectedPrefabs[i] };
                    }
