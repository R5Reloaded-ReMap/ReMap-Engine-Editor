...
public class Helper
{
    ...
    /// </summary>
    public static readonly Dictionary< ObjectType, bool > ObjectsToHide = new(GenerateObjects);
    //  
    ...
    public static readonly ObjectType[] GenerateIgnoreStatic =
    {
    ...
    private static readonly Dictionary< string, string > LocalizedString = new()
    {
        { "#FUNCTION_NAME_PRECACHE", $"{functionName}_Init" },
    ...
    {
        return MenuInit.IsEnable( OffsetMenuOffset );
    }

    public static bool ShowStartingOffset()
    {
        return MenuInit.IsEnable( OffsetMenuShowOffset );
    }

    public static void SetUseStartingOffset( bool value )
    {
        MenuInit.SetBool( OffsetMenuOffset, value );
    }

    public static void SetShowStartingOffset( bool value )
    {
        MenuInit.SetBool( OffsetMenuShowOffset, value );
    }
    ...
    {
        var scene = UnityInfo.GetAllGameObjectInScene();

        int min = 0;
        int max = scene.Length;
        float progress = 0.0f;
    ...
    public static void CreateDataTableItem( NewDataTable dt, Object loadedPrefabResource )
    {
        ...
        var parent = GameObject.Find( dt.Collection );
        if ( parent != null )
    ...
    {
        var objectTypeArray = forceShow ? GetAllObjectType().Where( objectType => !array.Contains( objectType ) ).ToList() : array.ToList();
    ...
    {
        string nameRef = GetObjRefWithEnum( objectType );

        return ObjectsInScene.Any( obj => obj.name.Contains( nameRef ) );
    }

    public static GameObject[] GetAllObjectTypeInScene( bool selection = false )
    {
        return GetAllObjectTypeWithEnum( GetAllObjectType(), selection );
    }
    ...
    public static GameObject[] GetAllObjectTypeWithEnum( ObjectType objectType, bool selectionOnly = false )
    {
        if ( selectionOnly )
        {
            string tag = GetObjTagNameWithEnum( objectType );

            var selectedObject = Selection.gameObjects
        ...
        return UnityInfo.GetAllGameObjectInScene().Where( obj => obj.CompareTag( GetObjTagNameWithEnum( objectType ) ) ).ToArray();
    }
    ...
    public static GameObject CreatePath( string pathString, GameObject obj = null )
    {
        ...
        var folder = GameObject.Find( pathString );
        string path = "";
    ...
    {
        stopwatch.Stop();
        Ping( "Time Elapsed:", stopwatch.ElapsedMilliseconds, "ms" );
    }
