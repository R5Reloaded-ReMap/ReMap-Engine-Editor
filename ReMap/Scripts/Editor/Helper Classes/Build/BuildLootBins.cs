...
namespace Build
{
    public class BuildLootBin
    {
        public static async Task< StringBuilder > BuildLootBinObjects( GameObject[] objectData, BuildType buildType )
        {
            ...
            {
                var script = ( LootBinScript )Helper.GetComponentByEnum( obj, ObjectType.LootBin );
                ...
                switch ( buildType )
                {
                    ...
                    case BuildType.LiveMap:
                        LiveMap.AddToGameQueue( $"MapEditor_CreateLootBin( {Helper.BuildOrigin( obj, false, true )}, {Helper.BuildAngles( obj )}, {skin}, true )" );
                        break;
                }
            }
