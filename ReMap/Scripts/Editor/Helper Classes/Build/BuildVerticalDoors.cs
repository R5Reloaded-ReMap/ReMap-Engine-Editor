...
namespace Build
{
    public class BuildVerticalDoor
    {
        public static async Task< StringBuilder > BuildVerticalDoorObjects( GameObject[] objectData, BuildType buildType )
        {
            ...
            {
                var script = ( VerticalDoorScript )Helper.GetComponentByEnum( obj, ObjectType.VerticalDoor );
                ...
                switch ( buildType )
                {
                    case BuildType.Script:
                        AppendCode( ref code,
                            $"    MapEditor_SpawnDoor( {Helper.BuildOrigin( obj ) + Helper.ShouldAddStartingOrg()}, {Helper.BuildAngles( obj )}, eMapEditorDoorType.Vertical, false, {Helper.BoolToLower( script.AppearOpen )} )" );
                        break;
                    ...
                    case BuildType.LiveMap:
                        LiveMap.AddToGameQueue( $"MapEditor_SpawnDoor( {Helper.BuildOrigin( obj, false, true )}, {Helper.BuildAngles( obj )}, eMapEditorDoorType.Vertical, false, {Helper.BoolToLower( script.AppearOpen )}, true )" );
                        break;
                }
            }
